...
function XeninUI:DrawCircle(sx, sy, radius, vertexCount, color, angle)
    local vertices = {}
    local ang = -rad(angle or 0)
    local c = cos(ang)
    local s = sin(ang)
...
/*
function XeninUI:DrawRoundedBox(bordersize, x, y, w, h, color)
    return XeninUI:DrawRoundedBoxEx(bordersize, x, y, w, h, color, true, true, true, true)
end

function XeninUI:DrawRoundedBoxEx(bordersize, x, y, w, h, color, a, b, c, d)
    x = math.Round(x)
    y = math.Round(y)
    w = math.Round(w)
    h = math.Round(h)
