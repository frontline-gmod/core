...
function XeninUI:DrawNPCOverhead(npc, tbl)
  ...
  local h = 64 * 3
	...
	XeninUI:DrawRoundedBox(h / 2, x, y, width, h, ColorAlpha(col, alpha))
  XeninUI:DrawRoundedBox(h / 2, x + 8, y + 8, width - 16, h - 16, ColorAlpha(XeninUI.Theme.Background, alpha))

	local textX =  !icon and (width / 2) or h
  draw.SimpleText(str, "XeninUI.NPC.Overhead", x + textX + textOffset, h / 2 + y, Color(225, 225, 225, alpha), icon and TEXT_ALIGN_LEFT or TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
